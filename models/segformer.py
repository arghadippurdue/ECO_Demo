...
class CustomConvModule(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, norm_type='BN', requires_grad=True):
        ...
        elif norm_type == 'GN':  # If you want GroupNorm as an alternative
            self.norm = nn.GroupNorm(num_groups=32, num_channels=out_channels)
        else:
            self.norm = None  # No normalization
        
        self.activation = nn.ReLU(inplace=True)  # Default activation function

        # Set requires_grad if needed
        if self.norm and not requires_grad:
...
class SegFormerHead(nn.Module):
    ...
    def __init__(self, feature_strides=None, in_channels=128, embedding_dim=256, num_classes=20, **kwargs):
        ...
        # self.linear_fuse = CustomConvModule(
        #     in_channels=embedding_dim*4,
        #     out_channels=embedding_dim,
        #     kernel_size=1,
        #     norm_type='BN',
    ...
    def forward(self, c1, c2, c3, c4):
        ...
        # _c = torch.mean(torch.stack([_c4, _c3, _c2, _c1], dim=1), dim=1)
...
class WeTr(nn.Module):
    ...
    def forward(self, input1, input2):   # c11, c12, c13, c14, c21, c22, c23, c24):
        ...
        ## Adding encoder
        x, _ = self.encoder([input1, input2])
        c11, c12, c13, c14 = x[0]
        c21, c22, c23, c24 = x[1]
        x1 = self.decoder(c11, c12, c13, c14)
        x2 = self.decoder(c21, c22, c23, c24)
        alpha_soft = F.softmax(self.alpha, dim=0)
        ens = alpha_soft[0] * x1.detach() + alpha_soft[1] * x2.detach()
        return [x1, x2, ens]
    # , masks
